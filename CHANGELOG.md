...
- Unspecced / follow-you shows pick-a-specialization (never “run the scrape”), and the Character bar uses short “No Spec”
- `GetPlayerSpecID` treats `0` as no-spec, and mid-respec is debounced so talent edits don’t reset build tab / Compare
- Build memory is per class/spec (`lastVariantByKey`)
- BiS quality borders + Myth bonuses (`12806`/`13335`) resolve from the display hyperlink, and incomplete tags complete either way
- Compare picks the first pair of differing builds, and titles show Raid/M+ tags
- DR meter lays out after next-breakpoint lines, hovers fall back to Midnight defaults, and spell tokens render gold
- Item `(true N)` skips equipped gear and tags once per tooltip paint
- Empty states wrap, narrow gear footers use “Alts”, and `/spf status` says “updated”
- Panel saves `relativePoint`, and minimap drag no longer toggles the panel on release
- Character bar opens the panel when unspecced, preferring open panel / remembered build
...
- Compact priority bar above PaperDoll, with soft ElvUI / EllesmereUI skins when those suites theme the character sheet
...
- Weekly Wowhead scrape under `tools/` (`browser_scrape.js` + import), and `/spf status` reports `missingDR` / `missingGear`
